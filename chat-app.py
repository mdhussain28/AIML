...
def detect_intent(message: str):
    ...
    if "balance" in msg:
        ...
        return "loan_query"
    elif "fraud" in msg or "stolen" in msg:
        return "fraud_alert"
    elif "lost" in msg:
        return "card_issue"
...
def generate_reply(intent: str, risk_flag: str):
    ...
    return responses.get(intent, "Support request received.")
...
def chat(message: str = "hello"):
    ...
    try:
        ...
        model_acc = model.get("accuracy", 0.0)
    except:
        pass
    try:
        risk = requests.get(
    ...
    reply = generate_reply(intent, risk.get("flag", "unknown"))
    ...
    try:
        with open(f"{MODEL_DIR}/memory.json", "w") as f:
            json.dump(conversation_memory[-100:], f)  # keep last 100
    except:
        pass
